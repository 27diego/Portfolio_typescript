...
export const Pannel: React.FC<Projects> = ({ title, logo, icons, landing }) => {
  const [expand, setExpand] = useState<boolean>(false);
  const [show, setShow] = useState<boolean>(false);

  useEffect(() => {
    if (title === "Monterey Mushrooms") {
      setExpand(true);
      setShow(true);
    }
  }, [title, setExpand, setShow]);
    ...
    >
      ...
      {show ? (
        <div className="content">
          ...
          <figure className="project__landing--wrapper">
            <img
              className="project__landing"
              src={landing}
              alt="landing page"
            />
          </figure>
